...
function flws_get_fonts(){
	//check if we are using built in google fonts or a custom source
	$use_custom = get_field('flws_use_custom_font','option');
	if($use_custom){
		$custom_font_src = get_field('flws_custom_font_script','option');
		$custom_font_style = get_field('flws_custom_font_styles','option');
		$font_call = $custom_font_src.'<style>'.$custom_font_style.'</style>';
	} else {
